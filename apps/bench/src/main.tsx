import { createSignal, createSelector, batch, untrack, For } from "atomic-duck";
import { render } from "atomic-duck";
...
render(() => {
  const [data, setData] = createSignal<{id: string, label: () => string, setLabel: (x: string) => void}[]>([]);
  const [selected, setSelected] = createSignal(null);
  const run = () => setData(buildData(1_000));
  const runLots = () => setData(buildData(10_000));
  const add = () => setData([...data(), ...buildData(1_000)]);
  const update = () =>
    batch(() => {
      for (let i = 0, d = data(), len = d.length; i < len; i += 10) d[i].setLabel(d[i].label() + " !!!");
    });
  const clear = () => setData([]);
  const swapRows = () => {
            ...
            {(row) => {
              ...
              return (
                <tr class={isSelected(rowId) ? "danger" : ""}>
                  ...
                  <td class="col-md-1">
                    <a onClick={() => {
                      setData(data().filter(({ id }) => id != row.id));
                    }}>
